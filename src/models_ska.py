    ...
    def __init__(self, block, layers, client, out_feature, groups=1, width_per_group=64):
        ...
        if True:
            ...
            self.maxpool = nn.MaxPool2d(
                kernel_size=3, stride=2, padding=1) if 'imagenet' == 'imagenet' else None
        else:
        ...
        if len(layers) > 3:
            ...
            self.layer4 = self._make_layer(block, base_inplanes*8, layers[3], stride=2, # stride change to 1
                                           dilate=replace_stride_with_dilation[2],
                                           replace_stride_with_avgpool=replace_stride_with_avgpool)
            outplanes = base_inplanes*8*block.expansion
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        if client == False: # Global model, no classifier 
            self.fc = nn.Identity()
        else:
            self.fc = nn.Sequential(
                nn.Linear(outplanes, 1024),
                nn.ReLU(),
...
def _resnet(arch, inplanes, planes, pretrained, progress, client, out_feature, **kwargs):
    model = ResNet(inplanes, planes, client, out_feature, **kwargs)
    if pretrained:
...
def resnet50(pretrained=False, progress=True, client=False, out_feature=1000, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress, client, out_feature,
                   **kwargs)
