...
class ResNet_IBN(nn.Module):
    ...
    def __init__(self, last_stride, block, layers, client, out_feature):
        scale = 64
        ...
        #self.outplanes = scale * 8 * block.expansion
        if client == False:
            self.fc = nn.Identity()
        else:
...
# modify here
def resnet50_ibn_a(last_stride, pretrained=False, client=False, out_feature=1000, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_IBN(last_stride, Bottleneck_IBN, [3, 4, 6, 3], client, out_feature, **kwargs)
    if pretrained:
        model.load_state_dict(torch.load("r50_ibn_a.pth"), strict=False)
    return model
